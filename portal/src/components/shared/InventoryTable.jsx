...
const INVENTORY_SORTS = [
  ...
  { value: 'condition_best', label: 'Condition (best first)',     col: 'condition', dir: 'asc'  },
  { value: 'age_youngest',   label: 'Age Range (youngest first)', col: 'age_range', dir: 'asc'  },
]
...
// ── Main Inventory ─────────────────────────────────────────────
export default function InventoryTable({ chapterId }) {
  ...
  const { books: rawBooks, count, totalPages, loading, memberMap, totals, addBook, updateBook, logDistribution } = useBooks(filters, page, activeChapterId)
  ...
  function handleSort(col) {
    ...
    const match = INVENTORY_SORTS.find(s => s.col === col && s.dir === newDir)

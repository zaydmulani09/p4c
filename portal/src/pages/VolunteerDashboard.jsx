...
export default function VolunteerDashboard() {
  ...
  const cards = [
    { icon: 'fa-book',          label: 'Log Books',      desc: 'Record book donations received' },
    { icon: 'fa-truck',         label: 'Distributions',  desc: 'Log a book distribution'        },
    { icon: 'fa-comment-dots',  label: 'Contact Log',    desc: 'Record outreach contact'         },
    { icon: 'fa-folder-open',   label: 'Resources',      desc: 'Access chapter guides'           },
  ]
    ...
    <div style={{ minHeight: '100vh', background: 'var(--primary-color)' }}>
      ...
      <div style={{ padding: '8rem 8% 5rem', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div className="fade-up" style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <div className="hashtag-badge" style={{ display: 'inline-flex', marginBottom: '1rem' }}>V O L U N T E E R</div>
            <h1 style={{ fontFamily: 'var(--font-heading)', fontWeight: 900, fontSize: 'clamp(2rem, 4vw, 3rem)', color: 'white', letterSpacing: '-0.02em' }}>
              Volunteer Portal
            </h1>
            {user && (
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '1rem', color: 'rgba(255,255,255,0.5)', marginTop: '0.4rem' }}>
                Welcome, <strong style={{ color: '#F6AA3C' }}>{user.email}</strong>
              </p>
            )}
          </div>
        ...
        <div className="fade-up-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1.2rem' }}>
          {cards.map(c => (
        ...
        <p className="fade-up-3" style={{ fontFamily: 'var(--font-accent-serif)', fontStyle: 'italic', fontSize: '1.1rem', color: 'rgba(255,255,255,0.3)', textAlign: 'center', marginTop: '4rem' }}>
          Full volunteer modules coming in Phase 2.
        </p>
      </div>
    </div>
  )
